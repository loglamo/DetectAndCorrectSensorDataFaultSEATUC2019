...
data_driff <- read.table(file ="~/Fault_Detecting/fault_detect/bias_656.txt",sep=",",row.names=1,header=TRUE )
...
par()
par(opar <- par())
plot(index, data.drift, type="n", main="bias_16", cex.main=1.0,  cex.lab=1.0, cex.axis=1.0) # draw plot
...
par()
par(opar <- par())
plot(index_result, result[1:2000], type="n", main="correct_16", cex.main=1.0,  cex.lab=1.0, cex.axis=1.0) # draw plot
