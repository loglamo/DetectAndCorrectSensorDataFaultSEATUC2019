...
data_clean<-read.table(file="~/Fault_Detecting/fault_detect/clean_drift_2.txt",sep=",",row.names=1,header=TRUE)
...
index <- c(1:1000) #take index
par()
par(opar <- par())
plot(index, data.drift[1:1000], type="n", main="", ylab="Temperature",xlab="Points of data", cex.main=1.0,  cex.lab=1.0, cex.axis=1.0) # draw plot
lines(index, data.drift[1:1000], type="l",col="blue") # draw line in plot
...
for(i in c(601:1000)){
  ...
  if((data.real < (data.forecastI$lower[,2]-3))||(data.real > (data.forecastI$upper[,2]+1))){
    print("Having fault............")
